...
bot.on("message", (messages) => {
  ...
  if (messages.content.substring(0, 1) == "!") {
            ...
            tf.loadGraphModel("file://./model/model.json").then((model) => {
              ...
              model
                ...
                .then((predictions) => {
                  ...
                  botSendMessage(
                    "__**Grading Chili Predictions:**__" +
                      predictionStringBuilder
                  );
                });
            });
      ...
      case "detectPneumonia":
        if (messages.attachments.size > 0) {
          ...
          download.image(options).then(({ filename }) => {
            ...
            tf.loadGraphModel("file://./model/pneumonia/model.json").then(
              (model) => {
                // Show tensor
                console.log("Tensor", tensor);
                ...
                model
                  ...
                  .then((predictions) => {
                    ...
                    botSendMessage(
                      "__**Pneumonia Detection Predictions:**__" +
                        predictionStringBuilder
                    );
                  });
              }
          ...
          let noImageText =
            "Please attach a chest X-ray image for detecting pneumonia.";
          noImageText +=
            "\n\n__**Disclaimer:**__ \n**1) I am not a medical professional nor a doctor, nor do I endorse the use of this pneumonia detection model to determine whether you have pneumonia, please consult a doctor and ask for a second or third opinion on another doctor.**";
          noImageText +=
            "\n\n**2) Please take this pneumonia detection model as a grain of salt.**";
    ...
    function addTwoValue(message) {
      var additionValue = message.content
        .replace("!add2value", "")
        .trim()
        .split(/[ ,]+/);
      ...
      if (isNaN(value1) || isNaN(value2)) {
        ...
        if (isNaN(value1) && isNaN(value2)) {
          ...
          invalidNumberTextBuilder =
            additionValue[0] + " and " + additionValue[1];
        ...
        invalidNumberTextBuilder +=
          " is not a number, kindly enter a valid number for " +
          invalidValueTextBuilder +
          ".";

        botSendMessage(
          invalidNumberTextBuilder + " <@" + messages.author.id + ">" + "."
        );
      } else {
        botSendMessage(
          "The addition of " +
    ...
    function listofPikaBotCommand() {
      ...
      // Pika Bot
      pikaCommandBuilder +=
        "__**Below are the available command for Pika Bot**__";
      pikaCommandBuilder += "\n__**Pika Bot**__";
      pikaCommandBuilder +=
